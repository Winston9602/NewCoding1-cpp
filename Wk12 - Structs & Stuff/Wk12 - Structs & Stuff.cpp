...
// Structures...
	// They are identical to classes in C++ (except public by default)
	// We can talk about POD (Plain Old Data) structs - they don't have functions in them
...
struct employee {
	...
	float wage;										//NOTE: you must define the struct you are calling before (above) you call it
...
int main() {
	...
	for (int i = 0; i < enemies.size(); i++) {
		...
		switch (enemies[i].type)				//kind of an alternative for an else/if series...
		{										//this only does one test rather than testing everything
		case GRUNT:
			cout << "grunt";
			break;
		case MINIBOSS:
